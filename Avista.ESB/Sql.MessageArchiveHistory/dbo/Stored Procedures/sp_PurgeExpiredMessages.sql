...
DECLARE @DeleteCount FLOAT = (SELECT COUNT(*) FROM [MessageArchiveHistory].[dbo].[Message] WITH(NOLOCK) WHERE ExpiryDate < @CurrentDate)
...
WHILE @LoopCtr < @MaxLoopCount AND (SELECT COUNT(*) FROM [MessageArchiveHistory].[dbo].[Message] WITH(NOLOCK) WHERE ExpiryDate < @CurrentDate) > 0
BEGIN
	DELETE FROM @MessageIds
	
	INSERT INTO @MessageIds (MessageId)
	SELECT TOP 100 MessageId FROM [MessageArchiveHistory].[dbo].[Message] WITH(NOLOCK) WHERE ExpiryDate < @CurrentDate
	
	SET @LoopCtr = @LoopCtr + 1
	DELETE FROM [MessageArchiveHistory].[dbo].[Message] WITH (ROWLOCK) WHERE MessageId IN (SELECT MessageId FROM @MessageIds)
