...
namespace Avista.ESB.Resolvers.Archive
{
    /// <summary>
    ///  Uses Archive Resolver to configure archive tag details ArchiveMessage Services.
    /// </summary>
    public class ArchiveResolver : IResolveProvider
    {
        ...
        public Dictionary<string, string> Resolve(string config, string resolver, IBaseMessage message, IPipelineContext pipelineContext)
        {
            ...
            {
                Exception exception = new Exception("Error performing Archive resolution in pipeline context.", ex);
        ...
        public Dictionary<string, string> Resolve(string config, string resolver, XmlDocument message)
        {
            ...
            {
                Exception exception = new Exception("Error performing Archive resolution in web service context.", ex);
                Logger.WriteTrace(string.Format("Error occured in {0} \r\n Details: {1}", this.GetType().Name, exception.ToString()));
                throw exception;
            }
        }
        ...
        public Dictionary<string, string> Resolve(ResolverInfo resolverInfo, XLANGMessage message)
        {
            ...
            {
                Exception exception = new Exception("Error performing Archive resolution in orchestration context.", ex);
                Logger.WriteTrace(string.Format("Error occured in {0} \r\n Details: {1}", this.GetType().Name, exception.ToString()));
                throw exception;
            }
            finally
        ...
        private static Dictionary<string, string> ResolveStatic(string config, string resolver, Resolution resolution)
        {
            ...
            try
            {
                ...
                // Retreive the values
                string expiryMinutes = ResolverMgr.GetConfigValue(queryParams, false, "expiryMinutes");
                string includeProperties = ResolverMgr.GetConfigValue(queryParams, false, "includeProperties");
                string failureEventId = ResolverMgr.GetConfigValue(queryParams, false, "failureEventId");
                string tag = ResolverMgr.GetConfigValue(queryParams, false, "tag");
                string failureAction = ResolverMgr.GetConfigValue(queryParams, false, "failureAction");
                ...
                resolverDictionary.Add("Archive.ExpiryMinutes", expiryMinutes.ToString() ?? "0");
                resolverDictionary.Add("Archive.IncludeProperties", includeProperties.ToString() ?? "true");
                resolverDictionary.Add("Archive.FailureEventId", failureEventId.ToString() ?? "324");
                resolverDictionary.Add("Archive.Tag", tag ?? "");
                resolverDictionary.Add("Archive.FailureAction", failureAction ?? "HandleException");
