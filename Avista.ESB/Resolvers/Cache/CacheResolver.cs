...
namespace Avista.ESB.Resolvers.Cache
{
    /// <summary>
    /// Uses Cache Resolver to configure Cache Services.
    /// </summary>
    public class CacheResolver : IResolveProvider
    {
        ...
        public Dictionary<string, string> Resolve(string config, string resolver, IBaseMessage message, IPipelineContext pipelineContext)
        {
            ...
            {
                Exception exception = new Exception("Error performing Cache resolution in pipeline context.", ex);
        ...
        public Dictionary<string, string> Resolve(string config, string resolver, XmlDocument message)
        {
            ...
            {
                Exception exception = new Exception("Error performing Cache resolution in web service context.", ex);
                Logger.WriteTrace(string.Format("Error occured in {0} \r\n Details: {1}", this.GetType().Name, exception.ToString()));
                throw exception;
            }
        }
        ...
        public Dictionary<string, string> Resolve(ResolverInfo resolverInfo, XLANGMessage message)
        {
            ...
            {
                Exception exception = new Exception("Error performing Cache resolution in orchestration context.", ex);
                Logger.WriteTrace(string.Format("Error occured in {0} \r\n Details: {1}", this.GetType().Name, exception.ToString()));
                throw exception;
            }
            finally
        ...
        private static Dictionary<string, string> ResolveStatic(string config, string resolver, Resolution resolution)
        {
            ...
            try
            {
                ...
                string action = ResolverMgr.GetConfigValue(queryParams, false, "action");
                string cacheMessageName = ResolverMgr.GetConfigValue(queryParams, false, "cacheMessageName");
                string cacheTimeOut = ResolverMgr.GetConfigValue(queryParams, false, "cacheTimeOut");
                ...
                resolverDictionary.Add("Cache.Action", action);
                resolverDictionary.Add("Cache.CacheMessageName", cacheMessageName);
                resolverDictionary.Add("Cache.CacheTimeOut", cacheTimeOut);
