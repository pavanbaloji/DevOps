...
namespace Avista.ESB.Utilities.Cache
{
    public class SqlBackingCache : ObjectCache
    {
        private string name = "Default"; //Default cache name
        private string connectionString;
        private string tableName = "Cache"; // Default table name
        private static readonly TimeSpan OneDay = new TimeSpan(24, 0, 0); // 1 day

        public SqlBackingCache() 
        {
            this.connectionString = ConfigurationManager.ConnectionStrings["CachingDb"].ToString();          
        }
        public SqlBackingCache(string connectionString) : this("Default", connectionString) { }

        public SqlBackingCache(string name, string connectionString)
        {
            if (name == null)
        ...
        public SqlBackingCache(string name, string connectionString, string tableName)
