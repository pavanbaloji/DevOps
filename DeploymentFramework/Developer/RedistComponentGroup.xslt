...
<xsl:stylesheet
  ...
  <xsl:template match="wix:File[contains(@Source,'UacElevate.exe')]">
      <xsl:copy>
        ...
        <xsl:processing-instruction name="if">$(var.IncludeSqlPackageDeployShortcut) ~= True</xsl:processing-instruction>
		<xsl:element name="Shortcut">
          <xsl:attribute name="Id">DeploySqlShortcut</xsl:attribute>
          <xsl:attribute name="Directory">BizShortCuts</xsl:attribute>
          <xsl:attribute name="Name">Deploy Sql Packages</xsl:attribute>
          <xsl:attribute name="Arguments">&quot;[MSBUILDPATH]&quot; &quot;/p:Configuration=Server /t:LaunchServerSqlPackageDeployWizard $(var.ProjectFilename) [MSBUILDTOOLSVER]&quot;</xsl:attribute>
          <xsl:attribute name="WorkingDirectory">DEPLOYMENTDIR</xsl:attribute>
          <xsl:attribute name="Advertise">yes</xsl:attribute>
        </xsl:element>
		<xsl:processing-instruction name="endif" />
